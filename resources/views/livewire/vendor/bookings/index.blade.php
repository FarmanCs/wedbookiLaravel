<div class="min-h-screen bg-gray-50 dark:bg-zinc-950" x-data="{ showTooltip: false }">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mt-2">Manage all your bookings and accept/reject requests
                    </p>
                </div>
            ...
            @if (session('success'))
                <div
                    class="mb-4 p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 flex items-center gap-3">
            ...
            @if (session('error'))
                <div
                    class="mb-4 p-4 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 flex items-center gap-3">
                    ...
                    <thead class="bg-gray-50 dark:bg-zinc-800/50 border-b border-gray-200 dark:border-zinc-800">
                        <tr>
                            <th class="px-6 py-4 text-left">
                                <button wire:click="updateSort('custom_booking_id')"
                                    class="text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2 hover:text-gray-900 dark:hover:text-white">
                                    Booking ID
                            ...
                            <th
                                ...
                                Host Name</th>
                            <th class="px-6 py-4 text-left">
                                <button wire:click="updateSort('event_date')"
                                    class="text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2 hover:text-gray-900 dark:hover:text-white">
                                    Event Date
                                    <flux:icon.arrows-up-down class="w-4 h-4" />
                                </button>
                            ...
                            <th class="px-6 py-4 text-left">
                                <button wire:click="updateSort('amount')"
                                    class="text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2 hover:text-gray-900 dark:hover:text-white">
                                    Amount
                                    <flux:icon.arrows-up-down class="w-4 h-4" />
                                </button>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-zinc-800">
                        @forelse($bookings as $booking)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
                                ...
                                <!-- Host Name -->
                                <td class="px-6 py-4">
                                    <p class="font-medium text-gray-900 dark:text-white">{{ $formatted['host_name'] }}
                                    </p>
                                ...
                                <td class="px-6 py-4">
                                    @if ($formatted['status_raw'] === 'pending')
                                        <span
                                            class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 text-xs font-medium">
                                            <span class="w-2 h-2 rounded-full bg-yellow-600 animate-pulse"></span>
                                            {{ $formatted['status'] }}
                                        </span>
                                    @elseif($formatted['status_raw'] === 'confirmed')
                                        <span
                                            class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-xs font-medium">
                                            <span class="w-2 h-2 rounded-full bg-green-600"></span>
                                            {{ $formatted['status'] }}
                                        </span>
                                    @elseif($formatted['status_raw'] === 'completed')
                                        <span
                                            class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-xs font-medium">
                                            <span class="w-2 h-2 rounded-full bg-blue-600"></span>
                                            {{ $formatted['status'] }}
                                        </span>
                                    @elseif($formatted['status_raw'] === 'rejected' || $formatted['status_raw'] === 'cancelled')
                                        <span
                                            class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-xs font-medium">
                                            <span class="w-2 h-2 rounded-full bg-red-600"></span>
                                            {{ $formatted['status'] }}
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-400 text-xs font-medium">
                                ...
                                <td class="px-6 py-4">
                                    <div>
                                        <p class="font-semibold text-gray-900 dark:text-white">Rs
                                            {{ $formatted['final_amount'] }}</p>
                                        <p class="text-xs text-gray-600 dark:text-gray-400">
                                            {{ $formatted['payment_status'] }}</p>
                                    </div>
                                </td>
                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        @if ($formatted['status_raw'] === 'pending')
                                            ...
                                            <button wire:click="openActionModal({{ $booking->id }}, 'accept')"
                                                ...
                                                <span
                                                    class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity">Accept</span>
                                            </button>
                                            ...
                                            <button wire:click="openActionModal({{ $booking->id }}, 'reject')"
                                                ...
                                                <span
                                                    class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity">Reject</span>
                                            </button>
                                        @endif
                                        ...
                                        <button wire:click="viewDetails({{ $booking->id }})"
                                            ...
                                            <span
                                                class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity">View
                                                Details</span>
                                        </button>
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-lg font-semibold text-gray-900 dark:text-white mb-1">No Bookings
                                            Found</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">You don't have any bookings
    ...
    @if ($showDetailsModal && $selectedBooking)
        <div class="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center p-4 z-50"
            ...
            <div
                ...
                <!-- Modal Header -->
                <div
                    class="sticky top-0 bg-gradient-to-r from-blue-500 to-blue-600 dark:from-blue-700 dark:to-blue-800 px-6 py-6 flex items-center justify-between border-b border-blue-600 dark:border-blue-900">
                    <div class="flex items-center gap-3">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div
                            class="bg-gradient-to-br from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 rounded-lg p-4">
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div>
                                    <p class="text-xs text-gray-600 dark:text-gray-400">Total Amount</p>
                                    <p class="font-bold text-lg text-gray-900 dark:text-white">Rs
                                        {{ $selectedBooking['amount'] }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-600 dark:text-gray-400">Advance %</p>
                                    <p class="font-bold text-lg text-gray-900 dark:text-white">
                                        {{ $selectedBooking['advance_percentage'] }}%</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-600 dark:text-gray-400">Advance Amount</p>
                                    <p class="font-bold text-lg text-gray-900 dark:text-white">Rs
                                        {{ $selectedBooking['advance_amount'] }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-600 dark:text-gray-400">Final Amount</p>
                                    <p class="font-bold text-lg text-gray-900 dark:text-white">Rs
                                        {{ $selectedBooking['final_amount'] }}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="bg-gray-50 dark:bg-zinc-800/50 rounded-lg p-4">
                                ...
                                <div class="flex items-center gap-2">
                                    @if ($selectedBooking['advance_paid'] === 'Paid')
                                        <flux:icon.check-circle class="w-5 h-5 text-green-600 dark:text-green-400" />
                                        <span
                                            class="font-semibold text-green-600 dark:text-green-400">{{ $selectedBooking['advance_paid'] }}</span>
                                    @else
                                        <flux:icon.clock class="w-5 h-5 text-yellow-600 dark:text-yellow-400" />
                                        <span
                                            class="font-semibold text-yellow-600 dark:text-yellow-400">{{ $selectedBooking['advance_paid'] }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="bg-gray-50 dark:bg-zinc-800/50 rounded-lg p-4">
                                ...
                                <div class="flex items-center gap-2">
                                    @if ($selectedBooking['final_paid'] === 'Paid')
                                        <flux:icon.check-circle class="w-5 h-5 text-green-600 dark:text-green-400" />
                                        <span
                                            class="font-semibold text-green-600 dark:text-green-400">{{ $selectedBooking['final_paid'] }}</span>
                                    @else
                                        <flux:icon.clock class="w-5 h-5 text-yellow-600 dark:text-yellow-400" />
                                        <span
                                            class="font-semibold text-yellow-600 dark:text-yellow-400">{{ $selectedBooking['final_paid'] }}</span>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            <div class="bg-gray-50 dark:bg-zinc-800/50 rounded-lg p-4">
                                ...
                                <p class="font-semibold text-gray-900 dark:text-white">
                                    {{ $selectedBooking['advance_due_date'] }}</p>
                            </div>
                            <div class="bg-gray-50 dark:bg-zinc-800/50 rounded-lg p-4">
                                <p class="text-xs text-gray-600 dark:text-gray-400">Final Due Date</p>
                                <p class="font-semibold text-gray-900 dark:text-white">
                                    {{ $selectedBooking['final_due_date'] }}</p>
                            </div>
                        </div>
                    </div>
    ...
    @if ($showActionModal && $selectedBooking)
        <div class="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center p-4 z-50"
            wire:key="action-modal">
            <div
                class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl max-w-md w-full border border-gray-200 dark:border-zinc-800">

                <!-- Modal Header -->
                <div
                    class="bg-gradient-to-r {{ $actionType === 'accept' ? 'from-green-500 to-green-600 dark:from-green-700 dark:to-green-800' : 'from-red-500 to-red-600 dark:from-red-700 dark:to-red-800' }} px-6 py-6 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-lg bg-white bg-opacity-20 flex items-center justify-center">
                            <flux:icon.{{ $actionType === 'accept' ? 'check' : 'x-mark' }}
                ...
                <div class="p-6 space-y-4">
                    <div class="space-y-2">
                        <h3 class="font-semibold text-gray-900 dark:text-white">{{ $selectedBooking['custom_id'] }}
                        </h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $selectedBooking['host_name'] }}</p>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">Rs
                            {{ $selectedBooking['final_amount'] }}</p>
                    </div>
                    ...
                    @if ($actionType === 'reject')
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Reason
                                (Optional)</label>
                            <textarea wire:model="actionReason" placeholder="Add a reason for rejection..."
                ...
                <div
                    ...
                    <button wire:click="{{ $actionType === 'accept' ? 'acceptBooking' : 'rejectBooking' }}"
